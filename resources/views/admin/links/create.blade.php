...
@section('content')
    <div class="max-w-2xl mx-auto bg-white shadow rounded-lg p-6">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Tambah Link Baru</h2>
        </div>
        ...
        <form action="{{ route('admin.links.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label for="parent_id" class="block text-sm font-medium text-gray-700">Kategori Induk (Lokasi Tautan)</label>
                    <select name="parent_id" id="parent_id"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="">Halaman Utama (Root)</option>
                        @foreach($subpages as $sub)
                            <option value="{{ $sub->id }}" {{ old('parent_id', request('parent_id')) == $sub->id ? 'selected' : '' }}>
                                Sub-Halaman: {{ $sub->title }}
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-gray-500">Pilih jika ingin menaruh link ini di dalam sub-halaman lain.</p>
                </div>

                <div class="flex flex-col justify-end pb-2">
                    <div class="flex items-center">
                        <input id="is_subpage" name="is_subpage" type="checkbox" value="1" {{ old('is_subpage') ? 'checked' : '' }}
                            class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        <label for="is_subpage" class="ml-2 block text-sm text-gray-900 font-semibold">Tipe: Sub-Halaman (Kategori Link)</label>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Centang jika link ini ketika di-klik akan membuka halaman baru berisi kumpulan link-link anak.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Max 2MB (jpg, png, svg). Ini akan menimpa pilihan FontAwesome.</p>
                </div>
            </div>
